        ...
        if (mysqli_num_rows($resultado) > 0) {
            ...
            if ($stmt_borrar = mysqli_prepare($conexion, $sql_borrar)) {
                ...
                if (mysqli_stmt_execute($stmt_borrar)) {
                    // Éxito
                    echo "<script>
                            alert('El síntoma \"$nombre\" fue dado de baja exitosamente.');
                            window.location.href = 'addSintomas.php';
                          </script>";
                } else {
                    echo "<script>alert('Error al intentar borrar de la base de datos.'); window.location.href = 'addSintomas.php';</script>";
                }
